...
savedir <- sprintf("/your/directory/path/%s_basin_%d", experiment_name, threshold)
delta_dir <- sprintf("/your/directory/path/delta/%s_basin_%d", experiment_name, threshold)  # delta保存用
...
# --- GAMLSS Fit ---
model_nonstat_true <- gamlss(
  outflow_neg ~ poly(year, 2, raw = TRUE),           # μ に year の 2次項まで
  sigma.fo = ~ poly(year, 2, raw = TRUE),            # σ にも year の 2次項まで
  family = GU,
  data = amax,
  trace = FALSE
)
